<script lang="ts">
	...
	const loadBookings = async (page: any, limit: any) => {
		loading = true
		try {
			const response = await api.getBookingsByUser(page, limit)
			bookings = [...bookings, ...response.data.bookings]
			total = response.data?.total_count
		} catch (err) {
			console.log(err)
			notificationCenter.displayErrorNotification(
				'Une erreur est survenue lors du chargement de mes réservations.'
			)
		} finally {
			loading = false
		}
...
<ViewBookingModal bind:isViewBookingModalOpened bind:selectedBooking {getUserInfo} />
