<script>
	...
	import ManageSettings from '../Admin/Contents/ManageSettings.svelte'
	...
	const onSendEmailVerification = async () => {
		if (foundEmail && isEmailVerified === false) {
			try {
				loading = true
				await api.sendEmailVerification(foundEmail, false)
...
<div class="md:py-20 px-8">
	{#if !isEmailVerified}
		<div class="flex flex-col items-center justify-center h-96 gap-8">
			<span>Oops! Votre email n'est pas encore vérifié.</span>
			<button
				class="text-xs btn btn-primary rounded-md"
				disabled={!foundEmail}
				on:click={onSendEmailVerification}
			>
				{#if loading}
					<div class="spin">
						<svg
