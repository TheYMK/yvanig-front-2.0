<script lang="ts">
	// @ts-ignore
	import Resizer from 'react-image-file-resizer';
	import { notificationCenter } from '../../config/notification';
	import { api } from '../../api/Api';
	import { toBase64 } from '../../config/helpers';

	export let images: any = [];
	export let placeholder = '';
	let loading = false;

	const fileUploadAndResize = async (e: any) => {
		let files = e.target.files;
		let uploadedFiles = images;
		loading = true;

		if (files) {
			for (let i = 0; i < files.length; i++) {
				const base64Image = await toBase64(files[i]);
				api
					.uploadImage(base64Image)
					.then((res) => {
						uploadedFiles.push(res.data);
						images = uploadedFiles;
					})
					.catch((err) => {
						console.log(err);
						notificationCenter.displayErrorNotification("Echec du téléchargement de l'image");
					})
					.finally(() => {
						loading = false;
					});
			}
		}
	};

	const handleRemove = (public_id: any) => {
		loading = true;

		api
			.removeImage(public_id)
			.then((res) => {
				let filteredImages = images.filter((image: any) => {
					return image.public_id !== public_id;
				});

				images = filteredImages;
			})
			.catch((err) => {
				console.log(err);
				notificationCenter.displayErrorNotification("Echec de la suppression de l'image");
			})
			.finally(() => {
				loading = false;
			});
	};
</script>

<div class="card shadow-lg my-8 cursor-pointer">
	<div class="card-body">
		<label
			class="flex flex-col w-full h-32 border-4 border-dashed hover:bg-gray-100 hover:border-gray-300"
		>
			<div class="flex flex-col items-center justify-center pt-7 cursor-pointer">
				{#if loading}
					<div class="spin">
						<svg
							role="status"
							class="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-white fill-primary"
							viewBox="0 0 100 101"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path
								d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
								fill="currentColor"
							/>
							<path
								d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
								fill="currentFill"
							/>
						</svg>
					</div>
					<p class="pt-1 text-sm tracking-wider text-gray-400 group-hover:text-gray-600">
						En cours de téléchargement...
					</p>
				{:else}
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="w-12 h-12 text-gray-400 group-hover:text-gray-600"
						viewBox="0 0 20 20"
						fill="currentColor"
					>
						<path
							fill-rule="evenodd"
							d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z"
							clip-rule="evenodd"
						/>
					</svg>
					<p class="pt-1 text-sm tracking-wider text-gray-400 group-hover:text-gray-600">
						{placeholder}
					</p>
				{/if}
			</div>
			<input
				type="file"
				multiple
				accept="images/*"
				class="opacity-0"
				on:input={fileUploadAndResize}
			/>
		</label>
	</div>
</div>

<div class="mt-8 w-full flex flex-col items-center">
	<div class="space-x-8">
		{#if images}
			{#each images as image}
				<div class="avatar indicator">
					<span
						class="indicator-item badge cursor-pointer"
						title="Supprimer l'image"
						on:click={() => handleRemove(image.public_id)}
					>
						X
					</span>

					<div
						class="mb-8 rounded-box w-28 h-28 ring ring-primary ring-offset-base-100 ring-offset-2"
					>
						<img src={image.url} alt="" />
					</div>
				</div>
			{/each}
		{/if}
	</div>
</div>

<style>
	/* your styles go here */
</style>
